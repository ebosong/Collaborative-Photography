...
class PlanValidator:
    ...
    def _clip_command(self, index: int, command: MotionCommand) -> MotionCommand:
        ...
        if command.target == "base":
            if command.action == "move":
                ...
                command.angular_z = self._clip_optional(
                    command.angular_z,
                    float(self.config["limits"]["base"]["max_angular_speed"]),
                    default=0.0,
                )
        ...
        if command.target == "lift":
            height_limits = self.config["limits"]["height_m"]
            if command.action == "move_to":
                command.height_m = self._clip_range(
                    command.height_m,
                    minimum=float(height_limits["min"]),
                    maximum=float(height_limits["max"]),
                    default=float(height_limits["default"]),
                )
            elif command.action == "move_by":
                command.delta_m = self._clip_optional(
                    command.delta_m,
                    float(self.config["limits"]["lift"]["max_delta_per_step"]),
                    default=0.0,
                )
            elif command.action == "stop":
        ...
        if command.target == "arm":
            if command.action == "preset" and not command.preset:
                command.preset = "ready"
    ...
    def _clip_duration(value: float) -> float:
        return round(max(0.0, min(30.0, float(value))), 2)
    ...
    def _safe_default_payload(self) -> dict[str, Any]:
        default_height = float(self.config["limits"]["height_m"]["default"])
        return {
            "script": {
