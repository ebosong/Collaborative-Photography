...
def translate_command(command: dict[str, Any], current_lift_height_m: float) -> tuple[list[dict[str, Any]], float]:
    """
    Translate one old command-format command into the current S3/P4 hardware protocol.

    This is a dry-run translator:
      - it does not open TCP
      - it does not connect to S3/P4
      - it only prints what would be sent
    """
    ...
    if target == "wait" or action == "wait":
        outputs.append({
            "note": f"wait locally for {float(command.get('duration_s') or 0.0):.3f}s"
        })
        return outputs, current_lift_height_m
